...
> SCP 条目、故事、GOI、Hub 的索引查询与正文获取。基于 D1 数据库 + Cloudflare KV 多级缓存架构。
...
| `offset` | number | 0 | 偏移量 |
| `series` | number | - | 按系列筛选（1-10） |
| `class` | string | - | 按对象等级筛选（Safe/Euclid/Keter/Thaumiel/Neutralized） |
| `search` | string | - | FTS5 全文搜索关键词 |
...
获取 SCP 条目正文内容。优先从 Cloudflare KV 缓存返回，未命中时回退到 GitHub Raw API 并写入 KV。

**Path Parameters**:

| 参数 | 类型 | 说明 |
|------|------|------|
| `number` | string | SCP 编号（如 `173`、`010`） |
...
`source` 取值：`"kv"`（命中 KV 缓存，<50ms）或 `"github-raw"`（回退获取）。
