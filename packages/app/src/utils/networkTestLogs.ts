...
export async function generateNetworkTestLogs(): Promise<string[]> {
  ...
  logs.push(
    `${ANSICode.red}║${ANSICode.reset}              ${ANSICode.green}Network Connectivity Test${ANSICode.reset}                  ${ANSICode.red}║${ANSICode.reset}`
  )
  logs.push(
    `${ANSICode.red}╚══════════════════════════════════════════════════════════════╝${ANSICode.reset}`
  )
  ...
  for (const target of TEST_TARGETS) {
    ...
    logs.push(
      `${ANSICode.gray}    Answer: ${generateRandomIP()} (TTL: ${Math.floor(Math.random() * 300 + 60)}s)${ANSICode.reset}`
    )
  ...
  for (const target of TEST_TARGETS) {
    logs.push(
      `${ANSICode.gray}  Testing ${target.name} (${target.host}:${target.port})...${ANSICode.reset}`
    )
  ...
  for (const target of TEST_TARGETS) {
    ...
    logs.push(
      `${ANSICode.gray}    ${statusColor}HTTP ${statusCode}${ANSICode.reset} ${ANSICode.gray}(${responseTime}ms)${ANSICode.reset}`
    )
    logs.push(`${ANSICode.green}    [OK]${ANSICode.reset}`)
    logs.push('')
  }
  ...
  for (const target of TEST_TARGETS) {
    ...
    for (let i = 1; i <= packetsSent; i++) {
      ...
      const status = isLost
        ? `${ANSICode.red}LOST${ANSICode.reset}`
        : `${ANSICode.green}RECV${ANSICode.reset}`

      if (i % 3 === 0 || i === packetsSent) {
        const progress = generateProgressBar(i, packetsSent, 25)
        logs.push(
          `${ANSICode.gray}    seq=${String(i).padStart(2)} time=${String(packetLatency).padStart(10)} ${status} ${progress}${ANSICode.reset}`
        )
      } else {
        logs.push(
          `${ANSICode.gray}    seq=${String(i).padStart(2)} time=${String(packetLatency).padStart(10)} ${status}${ANSICode.reset}`
        )
      }
    ...
    logs.push(
      `${ANSICode.gray}    ${packetsSent} packets transmitted, ${packetsReceived} received, ${lossPct}% packet loss, time ${latency}ms${ANSICode.reset}`
    )
  ...
  logs.push(
    `${ANSICode.red}║${ANSICode.reset}                  ${ANSICode.green}Network Test Summary${ANSICode.reset}                     ${ANSICode.red}║${ANSICode.reset}`
  )
  logs.push(
    `${ANSICode.red}╚══════════════════════════════════════════════════════════════╝${ANSICode.reset}`
  )
  ...
  logs.push(
    `${ANSICode.gray}  Download speed:    ${ANSICode.green}${downloadSpeed} Mbps${ANSICode.reset}`
  )
  logs.push(
    `${ANSICode.gray}  Upload speed:      ${ANSICode.green}${uploadSpeed} Mbps${ANSICode.reset}`
  )
  logs.push(
    `${ANSICode.gray}  Average latency:   ${ANSICode.green}${randomLatency(20, 80)}ms${ANSICode.reset}`
  )
  logs.push('')
  logs.push(
  ...
  logs.push(
    `${ANSICode.green}█${ANSICode.reset}  ${ANSICode.green}NETWORK HEALTHY - All connectivity tests passed${ANSICode.reset}            ${ANSICode.green}█${ANSICode.reset}`
  )
  logs.push(
    `${ANSICode.green}█${ANSICode.reset}                                                        ${ANSICode.green}█${ANSICode.reset}`
  )
