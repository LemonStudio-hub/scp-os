...
export function createApp(): Hono<AppEnv> {
  ...
  app.use('*', async (c, next) => {
    // Only resolve user id for rate-limit key when JWT_SECRET is configured.
    // Never fall back to a hardcoded signing secret.
  ...
  app.get('/scrape', async (c) => {
    ...
    if (!number)
      return json({ code: 'VALIDATION_ERROR', message: 'Missing number parameter', details: { field: 'number' } }, 400);
  ...
  app.get('/search', async (c) => {
    ...
    if (!keyword)
      return json(
        { code: 'VALIDATION_ERROR', message: 'Missing keyword parameter', details: { field: 'keyword' } },
        400,
      );
  ...
  app.get('/list', async (c) => {
    ...
    const data = await all(
      c.env.SCP_DB,
      `SELECT scp_id, name, object_class, tags, clearance_level, updated_at FROM scp_index WHERE ${where} ORDER BY scp_id ASC LIMIT ? OFFSET ?`,
      [...params, limit, offset],
    );
  ...
  app.get('/image-proxy', async (c) => {
    ...
    if (!value)
      return json({ code: 'VALIDATION_ERROR', message: 'Missing url parameter', details: { field: 'url' } }, 400);
    try {
      ...
      if (!allowed.some((host) => target.hostname === host || target.hostname.endsWith(`.${host}`))) {
        return json(
          { code: 'VALIDATION_ERROR', message: 'Image host not allowed', details: { host: target.hostname } },
          403,
        );
      }
      const response = await fetch(target.toString(), {
        headers: { Referer: target.origin, 'User-Agent': 'Mozilla/5.0 SCP-OS' },
      });
