...
export function registerUsers(app: Hono<AppEnv>): void {
  app.post('/api/user/register', async (c) => {
    ...
    const existing = await first<{ account_type: string }>(c.env.SCP_DB, 'SELECT account_type FROM users WHERE user_id = ?', [userId])
    const accountType = existing?.account_type === 'registered' ? 'registered' : 'guest'

    await run(
      c.env.SCP_DB,
      'INSERT INTO users (user_id, nickname, account_type, created_at, last_active_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET nickname = excluded.nickname, last_active_at = CURRENT_TIMESTAMP',
